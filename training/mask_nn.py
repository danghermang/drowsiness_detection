...
# script that trains neural network based on feature mask
INIT_LR = 1e-3
epochs = 3000
BS = 10
batch_size = 300
...
def train_generator_model(train_gen, x_test, y_test, steps, crossentropy=True, relu=True,
    ...
    checkpoint = keras.callbacks.ModelCheckpoint(MASK_MODEL_BEST_WEIGHTS, monitor='val_acc', verbose=1, save_best_only=True,
                                                 mode='max')
    ...
    with open(MASK_MODEL, "w") as json_file:
        json_file.write(model_json)
    model.save_weights(MASK_MODEL_WEIGHTS)
...
if __name__ == "__main__":
    ...
    # pickle.dump(history, open(MASK_MODEL_HISTORY, "wb"))
    #
    # history = pickle.load(open(MASK_MODEL_HISTORY, "rb"))
    # print(history)
    # show_plot(history)
    ...
    with open(MASK_MODEL, 'r') as json_file:
        loaded_model_json = json_file.read()

    loaded_model = keras.models.model_from_json(loaded_model_json)
    loaded_model.load_weights(MASK_MODEL_BEST_WEIGHTS)
    print(loaded_model.summary())

    history = train_generator_model(train_gen, x_test, y_test, batch_size=batch_size, model=loaded_model,
                                    epochs=epochs, steps=min(max_training_images_per_class * 2, steps_per_batch),
                                    patience=patience, crossentropy=crossentropy, relu=relu)
    pickle.dump(history, open(MASK_MODEL_BEST_HISTORY, "wb"))
    #
    # history = pickle.load(open(MASK_MODEL_BEST_HISTORY, "rb"))
    # print(history)
