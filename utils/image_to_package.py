...
# library that provides generators and different training related functions
...
def pre_load_images(training=True, width=128, max_img=10000, crossentropy=True):
    if training:
        image_path = PROCESSED_IMAGES_TRAINING
    else:
        image_path = PROCESSED_IMAGES_VALIDATION
    facial_shapes = []
    facial_shapes_tag = []
    ...
    for i, image in enumerate(files):
        ...
        if gray is False or gray is None:
            continue
        gray = common_functions.resize(gray, width)
...
def gen_load_images(training=True, width=128, batch_size=32,
                    max_type_img=10000, augumented=False, crossentropy=True):
    if training:
        image_path = PROCESSED_IMAGES_TRAINING
    else:
        image_path = PROCESSED_IMAGES_VALIDATION
...
def gen_load_mask(training=True, width=128, batch_size=32, max_type_img=10000, augumented=False, crossentropy=True):
    ...
    while True:
        ...
        for i, image in enumerate(files):
            ...
            gray = common_functions.resize(gray, width)
            if augumented:
                gray = common_functions.resize(aug.random_transform(gray), width)
            faces_detected = detector(gray, 0)
            if faces_detected:
                rect = faces_detected[0]
                shape = predictor(gray, rect)
